...
async def execute_set_away_now(
    ...
    return RedirectResponse(url=f"{request.scope.get('root_path', '')}/maindboard", status_code=303)
...
async def command_cancel(
    ...
    return RedirectResponse(url=f"{request.scope.get('root_path', '')}/commands", status_code=303)
