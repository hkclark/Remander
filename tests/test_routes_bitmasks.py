...
class TestBitmaskList:
    async def test_get_bitmasks_returns_200(self, client: AsyncClient) -> None:
        response = await client.get("/bitmasks")
        assert response.status_code == 200
        assert "Bitmasks" in response.text

    async def test_shows_hour_bitmask(self, client: AsyncClient) -> None:
    ...
    async def test_shows_zone_mask(self, client: AsyncClient) -> None:
...
class TestHourBitmaskDetail:
    async def test_get_hour_bitmask_detail(self, client: AsyncClient) -> None:
        bm = await create_hour_bitmask(
            name="Night Only", subtype=HourBitmaskSubtype.STATIC, static_value="0" * 8 + "1" * 16
        )
        response = await client.get(f"/bitmasks/hour/{bm.id}")
        assert response.status_code == 200
        assert "Night Only" in response.text

    async def test_hour_bitmask_not_found(self, client: AsyncClient) -> None:
        response = await client.get("/bitmasks/hour/999")
        assert response.status_code == 404


class TestHourBitmaskCreate:
    async def test_get_create_form(self, client: AsyncClient) -> None:
        response = await client.get("/bitmasks/hour/create")
        assert response.status_code == 200
        assert "form" in response.text.lower()

    async def test_post_create_static(self, client: AsyncClient) -> None:
...
class TestHourBitmaskEdit:
    async def test_get_edit_form(self, client: AsyncClient) -> None:
        ...
        response = await client.get(f"/bitmasks/hour/{bm.id}/edit")
        assert response.status_code == 200
        assert "Edit Me" in response.text

    async def test_post_edit(self, client: AsyncClient) -> None:
        bm = await create_hour_bitmask(
...
class TestHourBitmaskDelete:
    async def test_post_delete(self, client: AsyncClient) -> None:
        bm = await create_hour_bitmask(
...
class TestZoneMaskDetail:
    async def test_get_zone_mask_detail(self, client: AsyncClient) -> None:
        zm = await create_zone_mask(name="Full Zone", mask_value="1" * 4800)
        response = await client.get(f"/bitmasks/zone/{zm.id}")
        assert response.status_code == 200
        assert "Full Zone" in response.text

    async def test_zone_mask_not_found(self, client: AsyncClient) -> None:
        response = await client.get("/bitmasks/zone/999")
        assert response.status_code == 404


class TestZoneMaskCreate:
    async def test_get_create_form(self, client: AsyncClient) -> None:
        response = await client.get("/bitmasks/zone/create")
        assert response.status_code == 200

    async def test_post_create(self, client: AsyncClient) -> None:
        response = await client.post(
